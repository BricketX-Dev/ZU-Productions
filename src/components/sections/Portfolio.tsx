...
const CATEGORIES = ["All", "Music Video", "Commercial", "Event", "Documentary"];

// Helper to match projects with slugs defined in CASE_STUDIES
const getProjectSlug = (title: string) => {
  const lower = title.toLowerCase();
  if (lower.includes("ahmed")) return "ahmed-bukhatir";
  if (lower.includes("universal")) return "universal-brothers";
  if (lower.includes("jamia")) return "jamia-binoria";
  return null;
};
        ...
        <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project, idx) => {
              ...
              const destination = slug ? `/work/${slug}` : "/contact";
                  ...
                  <Link href={destination} className="block flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <div className="relative aspect-[16/9] w-full bg-neutral-950 border-b border-neutral-900 overflow-hidden flex items-center justify-center">
                        {/* Film Letterboxing */}
                        <div className="absolute top-0 left-0 right-0 h-3 bg-black/90 z-20" />
                        <div className="absolute bottom-0 left-0 right-0 h-3 bg-black/90 z-20" />

                        {/* Ambient Gradient Background */}
                        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-neutral-900 via-black to-black group-hover:scale-105 transition-transform duration-700" />

                        {/* Framing Reticles in Corners */}
                        <div className="absolute top-5 left-5 w-2 h-2 border-t border-l border-neutral-600 z-20" />
                        <div className="absolute top-5 right-5 w-2 h-2 border-t border-r border-neutral-600 z-20" />
                        <div className="absolute bottom-5 left-5 w-2 h-2 border-b border-l border-neutral-600 z-20" />
                        <div className="absolute bottom-5 right-5 w-2 h-2 border-b border-r border-neutral-600 z-20" />

                        {/* Top HUD Metadata */}
                        <div className="absolute top-4 left-8 z-20 font-mono text-[9px] text-neutral-500 uppercase tracking-widest">
                          2.39:1 // SCOPE
                        </div>
                        <div className="absolute top-4 right-8 z-20 font-mono text-[9px] text-brand-accent uppercase tracking-widest flex items-center gap-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-brand-red animate-ping" />
                          <span>4K RAW</span>
                        </div>

                        {/* Center Hover Action */}
                        <div className="relative z-10 w-12 h-12 rounded-full bg-neutral-900/90 border border-neutral-800 flex items-center justify-center text-neutral-400 group-hover:text-white group-hover:bg-brand-red group-hover:border-brand-red group-hover:scale-110 transition-all duration-300 crimson-glow">
                    ...
                    <div className="px-6 pb-6 pt-3 border-t border-neutral-900/80 flex items-center justify-between text-[11px] font-mono uppercase tracking-wider text-neutral-500 group-hover:text-white transition-colors">
                      <span className="flex items-center gap-1.5">
                        ...
                        <span>{slug ? "Case Study" : "Inquire Commission"}</span>
                      </span>
